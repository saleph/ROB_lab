function [networkOut terr] = backprop(tset, tslb, network, lr)
...
% terr - total squared error of the ANN
	...
	for i=numLayers-1: -1: 1
		d{i} = (d{i+1} * network{i}') .* [actdf(response{i}) ones(M, 1)];
		d{i} = d{i}(:, 1:end-1);
	...
	terr = 0.5 * sum((desiredOut - response{numLayers})(:).^2) / M;
