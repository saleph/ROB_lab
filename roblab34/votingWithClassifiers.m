function [ votes, classifiersVotes ] = votingWithClassifiers(tset, clsmx)
...
% votes - output matrix of votes cast by all one-versus-one classifiers
	...
	% classifiers score
	% classifiersVotes{:} - subsequent classifiers from clsmx
	% classifiersVotes{:}(:, 1:numel(labels)) - classifier votes for every sample
	classifiersVotes = {};
	...
	for i=1:size(clsmx,1)
		classifiersVotes{1,i} = clsmx(i,1:2);
		classifiersVotes{2,i} = zeros(size(votes));
		...
		votes(res >= 0, pid) += 1;
		classifiersVotes{2,i}(res >= 0, pid) = 9;
		...
		votes(res < 0, nid) += 1;
		classifiersVotes{2,i}(res < 0, nid) = 9;
	end
